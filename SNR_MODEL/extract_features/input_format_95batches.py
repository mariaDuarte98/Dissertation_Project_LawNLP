...
# this script was used to turn the each set of data ('trainAfterRules_26_07_2021_cor', 'valAfterRules_26_07_2021_cor', 'testAfterRules_26_07_2021_cor') from the prodigy annotation format to a file  in the format of the input of our system (with all concepts)
...
for file_name in files:
    ...
    for sentence in sentences:
        ...
        if len(sentence["spans"]) == 2:
            ...
            sent_ners_labels = {}
            for span in sentence["spans"]:
                if span["label"] not in sent_ners_labels.keys(): # start of category
                    ...
                    if span["token_start"] == len(sentence_token_withtag) - 1: #anotated on last #
                        if sentence_token_withtag[-2] in punctuation:
                            sentence_ners[index][1] = span["token_start"]  - 3 # remove punctuation and first and last #
                        else:
                            sentence_ners[index][1] = span["token_start"]  - 2 # remove first and last #
                        ...
                        sentence_ners[index][1] = span["token_start"]  - 2 # remove punctuation and first #
                        sent_ners_labels.pop(span["label"])  # found begining and end, done with the norm
                    else:
                        sentence_ners[index][1] = span["token_start"]  - 1
                        sent_ners_labels.pop(span["label"])  # found begining and end, done with the norm
            for ner in sentence_ners:
                if '-' in ner:
                    print("there is an error!!! - in spans")
        ...
        for rel in sentence["relations"]:
            if rel["label"] not in less100:
                if rel["head"] != len(sentence_token_withtag) - 1 and rel["child"] != len(sentence_token_withtag) - 1:
                    head = 1
                    child = 1
                    if rel["head"] < rel["child"]:
                        if sentence_token_withtag[rel["head"]] in punctuation: #if it starts with a punctuation, not include that token on the entity (next one)
                            head = 0
                            print("found punctuation!!!")
                            error = 1
                        if sentence_token_withtag[rel["child"]] in punctuation: #if it ends with a punctuation, not include that token on the entity (the one before)
                            child = 2
                            print("found punctuation!!!")
                            error = 1
                        sentence_ners.append([rel["head"] - head, rel["child"] - child, rel["label"]])
                    else:
                        if sentence_token_withtag[rel["child"]] in punctuation: #if it starts with a punctuation, not include that token on the entity (next one)
                            child = 0
                            print("found punctuation!!!")
                            error = 1
                        if sentence_token_withtag[rel["head"]] in punctuation: #if it ends with a punctuation, not include that token on the entity (the one before)
                            head = 2
                            print("found punctuation!!!")
                            error = 1
                ...
                # there is a sentence with a snr entity using the last #
                    ...
                    last = 2
                    if sentence_token_withtag[-2] in punctuation:  #making sure the last punctuation of the sentence isn't included in the entity (lref ...)
                        last = 3
                        print("found punctuation!!!")
                        error = 1
                    if rel["head"] == len(sentence_token_withtag) - 1: # uses last # in the relation #
                        if sentence_token_withtag[rel["child"]] in punctuation:
                            sentence_ners.append([rel["child"], rel["head"]  - last, rel["label"]])
                        else:
                            sentence_ners.append([rel["child"] - 1, rel["head"]  - last, rel["label"]])
                        
                    elif rel["child"] == len(sentence_token_withtag) - 1:  # uses last # in the relation #
                        if sentence_token_withtag[rel["head"]] in punctuation:
                            sentence_ners.append([rel["head"], rel["child"]  - last, rel["label"]])
                        else:
                            sentence_ners.append([rel["head"] - 1, rel["child"]  - last, rel["label"]])
    ...
    #size = 30
    file_name_model = file_name + '_READY.jsonl'
    ...
    #for i in range(0, len(sentences_ners), size):
        ...
        #json_examples = {"ners": sentences_ners,  "sentences": sentences_tokens, "ids": sentences_ids}
