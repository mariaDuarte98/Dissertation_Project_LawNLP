...
############# Definition of main parameters for the BERT model #############
...
def main(_):
    ...
    #json_examples = []
    #alteração
    file = FLAGS.input_file.split(';')[0]
    print(file)
    '''
    with open(file) as f:
        lines = f.readlines()
        json_examples = [json.loads(jline) for jline in lines]
        #json_examples.extend((json.loads(jsonline) for jsonline in lines))'''
        
    json_examples = [{"doc_key": "batch_01", "ners": [[[0, 0, "PER"], [3, 3, "GPE"], [5, 5, "GPE"]], [[3, 3, "PER"], [10,14, "ORG"], [20, 20, "GPE"], [20, 25, "GPE"], [22, 22, "GPE"]], []], "sentences": [["Anwar", "chegou", "a", "Shanghai", "de", "avião", "ontem", "tarde", "."], ["Esta", "manha", ",", "Anwar", "atendeu", "a", "fundação", "deixando", "cerimonia", "para", "o", "Minhang", "China-Malaysia", "da", "empresa", ",", "e", "depois", "disso", "tirou", "Pudong", "do", "lago", "para", "conseguir", "sobreviver", "."], ["(", "End", ")"]]}]
    ...
    print('hello')
    data = h5py.File('bert_features.hdf5', 'r')
